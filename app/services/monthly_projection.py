...
from app.models import MonthlyBudget, Transaction, TransactionStatus, TransactionType
...
def monthly_budget_positions(
    ...
    for budget in budgets:
        ...
        positions.append(BudgetPosition(budget=budget, spent=spent, remaining=amount - spent))
    return positions
...
def projection_budget_remaining(positions: list[BudgetPosition]) -> Decimal:
    return sum((max(position.remaining, Decimal(0)) for position in positions
                if position.budget.include_in_projection), Decimal(0))
