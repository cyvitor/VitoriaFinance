...
from app.services.telegram_expenses import parse_expense_command
...
def test_expense_command_parses_brazilian_and_decimal_amounts():
    assert parse_expense_command("/gasto 1.234,56 supermercado").amount == Decimal("1234.56")
    assert parse_expense_command("/gasto 25.90 cafe").amount == Decimal("25.90")
    assert parse_expense_command("/gasto zero cafe") is None


def test_expense_is_only_created_after_confirmation():
    with SessionLocal() as db:
        setup_linked_user(db)
        response = handle_message(db, message("/gasto 85,90 Mercado da esquina"))
        ...
        response = handle_message(db, message("/confirmar"))
        ...
        handle_message(db, message("/confirmar"))
        assert db.scalar(select(func.count(Transaction.id))) == 1


def test_expense_can_be_cancelled_and_expired_draft_is_not_confirmed():
    with SessionLocal() as db:
        setup_linked_user(db)
        handle_message(db, message("/gasto 20 lanche"))
        assert handle_message(db, message("/cancelar")) == "Gasto cancelado."
        assert db.scalar(select(func.count(Transaction.id))) == 0

        handle_message(db, message("/gasto 30 transporte"))
        ...
        db.commit()
        assert "expirou" in handle_message(db, message("/confirmar"))
        assert db.scalar(select(func.count(Transaction.id))) == 0
...
def test_unlinked_telegram_cannot_create_expense():
    with SessionLocal() as db:
        response = handle_message(db, {
            "text": "/gasto 100 mercado", "from": {"id": 999},
